...
bot.on('callback_query',async(callbackQuery) =>{
  ...
  switch(data){
    case 'get_weather':
      ...
      bot.sendMessage(chatId,'Please enter the name of the city or send /stop to cancel the process ')
      break
    case 'get_time':
      const userDataTime = getUserData(chatId)
      userDataTime.waitingForCity = true
      userDataTime.waitingForWeather = true
      bot.sendMessage(chatId,'Please enter the name of the city or  /stop to stop the process ')
      break;
    default:break;
  }
})
...
async function getTimeData(city){
  ...
  const response = await axios.get(`http://api.geonames.org/timezoneJSON?formatted=true&lat=${city.lat}&lng=${city.lon}&username=demo&style=full`)
